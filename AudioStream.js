...
module.exports = class AudioStream {
    ...
    newMPV(index) {
        let temp = exec(
            `mpv --cache=yes --no-video --force-window=no --cache-pause=yes --cache-pause-initial=yes --input-ipc-server=/tmp/mpvsocket${index} -`
        );
    ...
    rewind() {
        return exec(
            `echo '{ "command": ["seek", -10] }' | socat - /tmp/mpvsocket${this.current}`
        );
    }

    forward() {
        return exec(
            `echo '{ "command": ["seek", 5] }' | socat - /tmp/mpvsocket${this.current}`
        );
    }

    togglePauseResume() {
        return exec(
            `echo '{ "command": ["cycle", "pause"] }' | socat - /tmp/mpvsocket${this.current}`
        );
    }
    ...
    async extractVideoIdsFromPlaylist(playlistUrl) {
        ...
        const s = new Set();
        // Create a URL object from the playlist URL string
        const url = new URL(playlistUrl);

        // Extract the query string from the URL and create a URLSearchParams object
        // For example, a query string: ?list=PL0tOqbu4qXCgy_gynm3_SU7Zcq4HvgfTJ
        const urlParams = new URLSearchParams(url.search);

        // Get the value of the 'list' parameter (which is the playlist ID) from the URLSearchParams object
        const playlistId = urlParams.get("list");

        // Use in pagination when fetch the result from Youtube Data API
        // No token is needed for the first request
        let nextPageToken = "";
    ...
    async buffer(url) {
        ...
        return new Promise((resolve) => {
            mpv.stdout.on("data", (data) => {
                ...
                if (
                    data.toString().includes("Exiting") ||
                    data.toString().includes("underrun")
                ) {
    ...
    skip() {
        ...
        if (elapsedTime < 30 * 1000) {
            // 30 seconds put the amount of seconds you want on the left
            return false;
        } else {
            this.lastSkipTime = currentTime; // Update last skip time
            exec(
                `echo '{ "command": ["quit", "9"] }' | socat - /tmp/mpvsocket${this.current}`
            );
            return true;
        }
    }
